...
All these functions are strictly stateless, possessing no internal TF variables.
"""
...
def read_protein(filename_queue, max_length, num_edge_residues, 
    num_evo_entries, name=None):
    ...
    Returns:
        ...
        matrix_mask: Mask matrix (zero out pairwise distances in the masked regions)
    ...
    with tf.name_scope(name, 'read_protein', []) as scope:
        ...
        context, features = tf.parse_single_sequence_example(
            ...
            sequence_features={
                'primary': seq_feature(
                    (1,), tf.int64),
                'evolutionary': seq_feature(
                    (num_evo_entries,), tf.float32, allow_missing=True),
                'secondary': seq_feature(
                    (1,), tf.int64, allow_missing=True),
...
def curriculum_weights(base, slope, max_seq_length, name=None):
    ...
    Args:
        base: scalar TF tensor that is expected to change as training progresses.
...
def weighting_matrix(weights, name=None):
    ...
    Args:
        weights: Curriculum weights. A TF tensor that changes as curriculum progresses.
                 [MAX_SEQ_LENGTH - 1]
    ...
    with tf.name_scope(name, 'weighting_matrix', [weights]) as scope:
        ...
        for i in range(max_seq_length - 1):
            indices_subset = np.concatenate(
                (split_indices[0][:-(i+1), np.newaxis], split_indices[1][i+1:, np.newaxis]),
                1)
...
def id_filter(ids, filter_string, delimiter='#', name=None):
    ...
    with tf.name_scope(name, 'id_filter', [ids, filter_string]) as scope:
        ids = tf.convert_to_tensor(
            ids,
            name='ids')
        filter_string = tf.convert_to_tensor(
            filter_string,
            name='filter_string')
