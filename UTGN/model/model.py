...
class RGNModel(object):
    ...
    def _create_graph(self, mode, config):
        ...
        with tf.variable_scope(SCOPE, reuse=(mode == 'evaluation')) as scope, \
            ...
            positional_encodings = transformer._generate_positional_encodings(embed_dim, seq_len=max_length)

            inputs_mask = tf.ones((1, 1, step_dim), dtype=float)

            input_embeddings = transformer._prepare_embeddings(inputs2,
                                                  positional_encodings=positional_encodings,
                                                  keep_prob=keep_prob,
                                                 )
            recurrent_outputs = transformer._encoder(input_embeddings, mask=inputs_mask, n_layers=n_layers, heads=heads,
...
def _recurrence(mode, config, inputs, num_stepss):
    ...
    num_stepss: list with single element in it
    inputs: shape: [num_steps, batch_size, embedd_dim]
     """
